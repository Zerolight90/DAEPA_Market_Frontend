...
const primaryBtn: CSSProperties = {
    ...
    border: "none", borderRadius: 8, fontWeight: 700, fontSize: 15, cursor: "pointer", marginBottom: 10,
};
const secondaryBtn: CSSProperties = {
    width: "100%", padding: "12px 14px", background: "#1aab8a", color: "#fff",
    border: "none", borderRadius: 8, fontWeight: 700, fontSize: 15, cursor: "pointer", marginBottom: 10,
};
const ghostBtn: CSSProperties = {
    width: "100%", padding: "11px 14px", background: "#fff", color: "#333",
    border: "1px solid #ddd", borderRadius: 8, fontWeight: 600, fontSize: 14, cursor: "pointer",
};
const qtyBtn: CSSProperties = {
    width: 32, height: 32, border: "1px solid #ddd", borderRadius: 6,
    background: "#fff", cursor: "pointer", fontSize: 18, fontWeight: 700, lineHeight: 1,
};

export default function BuyModal({ id, close, itemId, title, price }: BuyModalProps) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await api.get<{ pdThumb?: string; thumbnail?: string }>(`/products/${itemId}`);
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                const res = await api.get<Address>("/sign/locations/default");
    ...
    const purchase = async () => {
        ...
        tossPayments
            .requestPayment("카드", { amount: total, orderId, orderName: title || "상품 구매", customerName: "id" })
            .then(({ paymentKey, amount }: { paymentKey: string; amount: number }) => {
                window.location.href = `${window.location.origin}/pay/success?paymentKey=${paymentKey}&orderId=${orderId}&amount=${amount}`;
            })
            .catch((error: { code?: string; message?: string }) => {
                if (error.code !== "USER_CANCEL") {
                    window.location.href = `${window.location.origin}/pay/fail?message=${error.message}`;
                }
            });
    };
            ...
            <div style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    ...
                    <button
                        ...
                        style={{ background: "none", border: "1px solid #ccc", borderRadius: 6, padding: "4px 10px", fontSize: 12, color: "#555", cursor: "pointer" }}
                    >
                        변경
                    </button>
                </div>
