...
import styles from "@/app/sign/login/login.module.css";
import axios from "axios";

export default function FindIdPage() {
    const [name, setName] = useState("");
    const [phone, setPhone] = useState("");
    const [result, setResult] = useState<any>(null); // 결과를 저장할 상태 (null, 'loading', {id: '...'..}, 'error')
    ...
    const handleFindId = async (e) => {
        ...
        try {
            ...
            const response = await axios.post("/api/sign/login/find_id", {
                u_name: name,
                u_phone: phone
            });
            ...
            <div className={styles.container}>
                <div className={styles.card}>
                    ...
                    <div className={styles.actions}>
                        <Link href="/sign/login" className={styles.submitBtn} style={{ textAlign: 'center' }}>
                            로그인하기
                        </Link>
                    </div>
                </div>
        ...
        <div className={styles.container}>
            <div className={styles.card}>
                ...
                <div className={styles.links}>
                    <Link href="/sign/login" className={styles.link}>로그인</Link>
                    <span className={styles.divider}>|</span>
                    <Link href="/sign/login/find_password" className={styles.link}>비밀번호 찾기</Link>
                </div>
            </div>
        </div>
    );
