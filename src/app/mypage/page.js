...
import { usePathname } from "next/navigation";
import styles from "./mypage.module.css";
import useTokenStore from "@/app/store/TokenStore"; // 수정된 스토어 import
import SideNav from "@/components/mypage/sidebar";
import { api } from "@/lib/api/client";
...
export default function MyPage() {
    ...
    const myProductsAll = useMemo(() => {
        ...
        if (!myId) return [];
        return products.filter((p) => {
            const owner = p.uIdx ?? p.u_idx ?? p.userIdx ?? p.user_idx ?? null;
            return owner != null && Number(owner) === Number(myId) && !isDeleted(p);
        });
    }, [products, myInfo.uIdx]);

    const myProductsSelling = useMemo(() => {
        return myProductsAll.filter((p) => [0, 2].includes(getDealState(p)));
    }, [myProductsAll]);
    ...
    const sortedItems = useMemo(() => {
        ...
        if (sort === "low") return base.sort((a, b) => (a.pd_price ?? 0) - (b.pd_price ?? 0));
        if (sort === "high") return base.sort((a, b) => (b.pd_price ?? 0) - (a.pd_price ?? 0));

        return base.sort((a, b) => parseDateSafe(b.pd_create ?? b.createdAt) - parseDateSafe(a.pd_create ?? a.createdAt));
    }, [tab, sort, myProductsAll, myProductsSelling, myProductsSold]);

    const trustVal = Number(myInfo.trust) || 0;
    const trustPercent = Math.max(0, Math.min(100, trustVal));
    const trustColor = trustPercent < 30 ? "#8B4513" : trustPercent < 60 ? "#A3E635" : "#10B981";
