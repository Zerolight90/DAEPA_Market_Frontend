...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 대시보드 통계 조회
        const statsRes = await fetch("http://localhost:8080/api/admin/analytics/stats");
        if (statsRes.ok) {
        ...
        // 일간 거래 추이 조회
        const transactionRes = await fetch("http://localhost:8080/api/admin/analytics/daily-transactions");
        ...
        // 최근 등록 상품 조회
        const productsRes = await fetch("http://localhost:8080/api/admin/analytics/recent-products?limit=5");
        if (productsRes.ok) {
