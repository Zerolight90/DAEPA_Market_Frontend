...
export default async function SellerPage({ params, searchParams }: PageProps) {
    ...
    const itemsWithDetail = await Promise.all(
        baseItems.map(async (it, idx) => {
            ...
            try {
                ...
                const detail = await fetchProductDetail(id) as any;
    ...
    const makeQuery = (next: { sort?: string; status?: string } = {}) => {
