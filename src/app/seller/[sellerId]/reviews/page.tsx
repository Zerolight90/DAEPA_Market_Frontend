...
export default function SellerReceivedReviewsPage() {
  ...
  // 판매자 후기 목록
  async function fetchReviews(targetId: string | string[], page = 0, append = false) {
    try {
      ...
      setList((s) => ({
        ...s,
        err: err.response?.data?.message || err.message || "네트워크 오류가 발생했습니다.",
        loading: false,
        initialized: true,
      }));
    }
  ...
  // 프로필 헤더(서버로 보정)
  async function fetchSellerHeader(targetId: string | string[]) {
    try {
